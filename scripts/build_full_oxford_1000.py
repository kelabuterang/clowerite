...
And binds them into src/data/dictionaryData.ts (1,040 total words).
"""
...
def main():
    print("Building Part 1 (A - D)...")
    part1_items = MERGED_PART1[:260]
    write_ts_file("src/data/oxfordPart1.ts", "OXFORD_PART_1", part1_items)

    print("Building Part 2 (E - L)...")
    from scripts.generate_oxford_part2_ts import WORDS_PART2
    seen2 = set()
    part2_items = []
    for it in WORDS_PART2 + PART2_WORDS + PART2_EXTRA:
        w = it[0] if isinstance(it, tuple) else it["word"]
        if w.lower() not in seen2:
            seen2.add(w.lower())
            part2_items.append(it)
    write_ts_file("src/data/oxfordPart2.ts", "OXFORD_PART_2", part2_items[:260])

    print("Building Part 3 (M - R)...")
    seen3 = set()
    part3_items = []
    for it in PART3_WORDS:
        ...
        if w.lower() not in seen3:
            seen3.add(w.lower())
            part3_items.append(it)
    write_ts_file("src/data/oxfordPart3.ts", "OXFORD_PART_3", part3_items[:260])

    print("Building Part 4 (S - Z)...")
    seen4 = set()
    part4_items = []
    for it in PART4_WORDS:
        ...
        if w.lower() not in seen4:
            seen4.add(w.lower())
            part4_items.append(it)
    write_ts_file("src/data/oxfordPart4.ts", "OXFORD_PART_4", part4_items[:260])

    # Write dictionaryData.ts
